...
-- OVERHAUL ----------------------
--[[
    Preset Content for 3 bars (Rep, Experience or Honor, Artifact)
    Create 3 bars and Hide
    Determine content for all 3 bars
    Fill up Content
    per event Update specific Bar
    return { min, max, value, bgColor, restedValue, restedBgColor, leftText, rightText}
]]
...
local function Bars()
    ...
    if (select(1, GetWatchedFactionInfo())) then
        table.insert("Reputation", bars)
    end
    if UnitLevel("player") < MAX_PLAYER_LEVEL then
        table.insert("Experience", bars)
    elseif UnitLevel("player") >= MAX_PLAYER_LEVEL then
        table.insert("Honor", bars)
    end
    local artifactItemID, _, _, _, artifactTotalXP, artifactPointsSpent, _, _, _, _, _, _, artifactMaxed = C_ArtifactUI.GetEquippedArtifactInfo()
    if artifactItemID and not artifactMaxed then
        table.insert("Artifact", bars)
    end
    return bars
end

local function CreateBar(barName, parent)
    local f = CreateFrame("BUTTON", "$parent."..barName, parent)
    f:SetSize(V.defaults.frame.width, module.barHeight)
...
local function Update:Reputation()
    local reputation
    local name, standing, min, max, value, factionID = GetWatchedFactionInfo()
    local color = FACTION_BAR_COLORS[standing]
    reputation = V.frames.bars.Reputation
...
local function Update:Experience()
    
end

local function Update:Honor()
    
end

local function Update:Artifact()
...
for _, bar in pairs(bars) do
    local bar = CreateBar(bar, baseFrame)
    baseFrame[bar] = bar
    module.height = module.height+bar:GetHeight()
    Update[bar]()
end